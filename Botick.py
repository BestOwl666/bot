...
bot = telebot.TeleBot('6075909461:AAE_lmqFiTuW_arXv6VJbWPZ8yUhDRYKf-s')

@bot.message_handler(commands=['start'])
def send_welcome(message):
    markup = types.InlineKeyboardMarkup()
    button1 = types.InlineKeyboardButton('GTA V', callback_data='button1')                   #ЕСТЬ
    button2 = types.InlineKeyboardButton('ЧИТ', callback_data='button2')                     #ЕСТЬ
...
def callback_query(call):
    ...
    if call.data == 'button2':
        bot.answer_callback_query(call.id, 'Запуск чита')
        pass
